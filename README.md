...
- 🤖 **부팅 후 자동 실행**: 오전 3시에 등록된 작업을 PC 부팅 후 Runner가 즉시 수신
...
### 자동 실행
- 매일 오전 3시(한국 시간)에 GitHub가 작업을 대기열에 등록합니다.
- 오전 3시에 PC가 꺼져 있어도 작업은 최대 24시간 대기합니다.
- PC가 부팅되어 Runner 서비스가 연결되면 대기 중인 작업이 즉시 실행됩니다.
- 관리자 PowerShell에서 `setup-runner-autostart.ps1`을 한 번 실행하면 Windows 시작 시 Runner가 자동 실행됩니다.
- PC를 24시간 이상 켜지 않으면 해당 작업은 만료되며 다음 날 작업이 새로 등록됩니다.
